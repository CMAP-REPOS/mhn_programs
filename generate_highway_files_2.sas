 ...
 *-------------------------------------------------------------------------------;
  ...
  *** -- Update toll values to January 2012 rates -- ***;
   %if &scen>100 %then %do; toll=round(toll*1.875,0.05); %end;

  *** -- Revert max speed limit to 65 MPH for pre-2014 scenarios -- ***;
   %if &scen=100 %then %do; posted1=min(posted1, 65); posted2=min(posted2, 65); %end;
...
data network2(drop=cn); set network;
    output;
     if directn=2 then do;
         cn=anode; anode=bnode; bnode=cn; parkres1=parkres2; resln1=resln2;
       output;
     end;
       proc sort; by anode bnode;
   ...
   if 1<=zone<=854 then area='01. Cook Co.';
    ...
    proc sort; by node;
       ...
       * - - - - - - - - - - - - - - - - - - - - - - - - - - *;
        ...
        data check; set check; if _freq_>1;
       ...
       data out2; set links&tod;
        ...
        if _n_= 1 then put 'c i-node,j-node,@speed,@width,@parkl,@cltl,@toll,@sigic,@rrx,@tipid';
          ...
          data out3; set netnodes;
           if node le &maxz then flag='a*'; else flag='a ';
             file out4;
           if _n_= 1 then do;
              put 'c a,node,x,y' /
                  't nodes init';
...
%macro report(timeper);
       ...
       data last; merge junk junk2; by area;
         label _freq_='Directional Links'
           ...
           proc print label noobs; var area node _freq_ miles lanemile cltlmi
                sigicmi tollmi parkmi trckmi;
                sum _freq_ miles lanemile node cltlmi sigicmi tollmi parkmi trckmi;
                format node _freq_ comma6. miles lanemile cltlmi sigicmi tollmi
                parkmi trckmi comma9.2;
