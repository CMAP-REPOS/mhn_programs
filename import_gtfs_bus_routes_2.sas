...
data pace2; merge pace(in=hit) node; by itinerary_a; if hit; proc sort; by itinerary_b;
...
data pace2; set pace2; by newline order;
    nl_lag = lag(newline);
    at_lag = lag(arr_time);
    discrep = at_lag - dep_time;
    ** Ensure segment starts when previous ends;
    if newline = nl_lag & discrep > 0 then do;
        dep_time + discrep;
        arr_time + discrep;
    end;
    ** Ensure time > 0 seconds. If time = 0, assume 30mph & Euclidean distance;
