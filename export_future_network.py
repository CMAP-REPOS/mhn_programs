...
out_gdb = '/'.join((gdb_path, gdb_name))
if not arcpy.Exists(out_gdb):
    arcpy.CreateFileGDB_management(gdb_path, gdb_name)

out_fd_name = 'hwynet_{0}'.format(build_year)
out_fd = '/'.join((out_gdb, out_fd_name))
...
update_link_csv = '/'.join((MHN.temp_dir, 'update_link.csv'))  # SAS output
flag_node_csv = '/'.join((MHN.temp_dir, 'flag_node.csv'))      # SAS output
...
temp_arc = '/'.join((MHN.mem, out_arc_name))
...
out_arc = '/'.join((out_fd, out_arc_name))
...
temp_node = '/'.join((MHN.mem, out_node_name))
...
out_node = '/'.join((out_fd, out_node_name))
