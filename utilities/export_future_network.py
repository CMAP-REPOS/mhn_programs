...
build_year = arcpy.GetParameter(1)  # Integer, default = 2050
...
out_fd_name = 'hwynet_{}'.format(build_year)
...
transact_query = "{} IN ('{}')".format(hwyproj_id_field, "','".join(id for id in hwy_projects))
...
sas1_sas = os.path.join(MHN.util_dir, '{}.sas'.format(sas1_name))
...
arcpy.AddMessage('\nBuilding {} links...'.format(build_year))
...
out_arc_name = 'hwynet_arc_{}'.format(build_year)
...
update_link_attr = [
    'ABB','DIRECTIONS','TYPE1','TYPE2','AMPM1','AMPM2','POSTEDSPEED1','POSTEDSPEED2',
...
arcpy.AddMessage('\nBuilding {} nodes...'.format(build_year))
...
out_node_name = 'hwynet_node_{}'.format(build_year)
