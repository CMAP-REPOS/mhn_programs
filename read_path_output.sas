/*
   ...
   revised: 6/10/15
   ----------------------------------------------------------------------------
   This program reads the shortest path data written by find_shortest_path.py
   and inserts it into the transit itineraries.
*/
options linesize=120;

** REFORMAT PATH DATA AS ITINERARY SEGMENTS **;
...
data first; set read; by set i;
    ...
    proc summary nway; class set; var itinerary_a itinerary_b;
        output out=ends max=;
...
data read; merge read (in=hit) len; by newa newb; if hit;
    proc summary nway; class set; var linkmi;
        output out=totlen sum=totmi;
...
proc sql noprint;
    create table newitin as
        select verify.*,read.newa,newb,set,i,impute,linkmi,totmi
...
%macro checkds(dsn);
    /* If dwadj exists: process to attach pnode1 to links so the correct dwell
        code can be assigned (highway links only) */
    %if %sysfunc(exist(&dsn)) %then %do;
        proc sql noprint;
            create table newitin2 as
                select newitin.*,dwadj.pnode1
...
data newitin; set newitin;
    retain d 0; 
    if imputed ^= i or line ^= group or dep_time ^= dt then d + 1;
    output;
    proc sort; by d rank;
