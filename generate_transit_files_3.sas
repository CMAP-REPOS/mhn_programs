...
data a; set itin;
   proc sql noprint;
    create table stop1 as
     select itin.itina, linename, dwcode,
            allcta.anode, bnode, dist
     from itin, allcta
...
data b; set itin;
   proc sql noprint;
    create table stop2 as
     select itin.itinb, linename, dwcode,
            allcta2.anode, bnode, dist
     from itin, allcta2
     where itin.itinb=allcta2.itinb;

**ELIMINATE NON-STOPS **;
data stop2; set stop2; if dwcode ne 1;
...
/*==============================================*/
/*   2. CREATE LINKS FOR u,v,w,x,y AND z        */
/*==============================================*/
  /* -------- u and x -------- */
            *-------------------------------------;
...
data dir1; set dir1;
 retain ord 1;
  ord+1;
  if linename ne group then ord=1;
 output;
...
data finlist; set finlist;
 retain ord 1;
  ord+1;
  if centroid ne match then ord=1;
 output;
...
data finlistu(drop=match ord t); set finlist;
 if ord>3 then delete;
   mode='u';
  t=stop; stop=centroid; centroid=t;
  output;
...
data force(drop=t); set force;
  mode='y';
  output;
   mode='v'; t=stop; stop=centroid; centroid=t;
  output;
...
data metra1(drop=t); merge mzone metra1; by stop centroid;
 ...
 if miles='.' then miles=.55;
  mode='z';
   output;
    t=stop; stop=centroid; centroid=t; mode='w';
   output;
